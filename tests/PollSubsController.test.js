...
describe('subscribeToPollResult', () => {
    ...
    test('should pass', async () => {
        ...
        const pollid = 1002;
        // Tests
        const reply = await PollSubsController.subscribeToPollResult(userid, pollid);
        // Expects
        expect(redClient.zadd).toHaveBeenCalledWith(keyPrefix.pollSubsription + pollid, userid, DATE_TO_USE.getTime());
...
describe('unSubscribeToPollResult', () => {
    ...
    test('should pass', async () => {
        ...
        const pollid = 1002;
        // Tests
        const reply = await PollSubsController.unSubscribeToPollResult(userid, pollid);
        // Expects
        expect(redClient.srem).toHaveBeenCalledWith(keyPrefix.pollSubsription + pollid, userid);
        expect(reply).toEqual(expectedResult);
    });
});
